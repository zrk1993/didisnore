...
export default class BaseService<T> {
  ...
  async getById(id: string | number): Promise<T> {
    if (!id) throw new Error(this.$primaryKey + ' null');
    return await db.table(this.$tableName).where({ [this.$primaryKey]: id }).findOrEmpty();
  }

  async getAll(): Promise<T[]> {
    return await db.table(this.$tableName).order(this.$primaryKey, 'DESC').select();
  }
  ...
  async removeByIds(keyProp: string, ids: string[]) {
    return this.$db.table(this.$tableName).where('?? in (?)', [keyProp, ids]).delete();
  }
}